...
type UserRow={email:string;name:string|null;identity:string|null;firstSeen:string;lastSeen:string;accessCount:number;videoViews:number;lastVideo:string|null};
...
export default function AdminPage(){
  ...
  const [identityFilter,setIdentityFilter]=useState("전체");
  const filteredUsers=useMemo(()=>data?.users.filter((user)=>{
    const viewedAt=new Date(user.lastSeen).getTime();
    const afterStart=!fromDate||viewedAt>=new Date(`${fromDate}T00:00:00`).getTime();
    const beforeEnd=!toDate||viewedAt<=new Date(`${toDate}T23:59:59.999`).getTime();
    const matchesIdentity=identityFilter==="전체"||(identityFilter==="미지정"?!user.identity:user.identity===identityFilter);
  ...
  const upload=async()=>{
    ...
    try{
      ...
      const metadata=await fetch("/api/admin/videos",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({title,description,minutes,mediaKey,audiences:selectedAudiences,levels:selectedLevels,topics:selectedTopics,tags})});
      if(!metadata.ok)throw new Error(await metadata.text());
      setProgress(100);setUploadStatus("신규 영상이 등록되어 추천과 영상 보관함에 반영되었습니다.");
      setFile(null);setTitle("");setDescription("");setTagInput("");setMinutes(5);
  ...
  return <main className="admin-page">
    ...
    <section className="admin-shell">
      ...
      <section className="admin-search-panel"><div><label>시작일<input type="date" value={fromDate} max={toDate||undefined} onChange={(event)=>setFromDate(event.target.value)}/></label><span>–</span><label>종료일<input type="date" value={toDate} min={fromDate||undefined} onChange={(event)=>setToDate(event.target.value)}/></label></div><label>신분<select value={identityFilter} onChange={(event)=>setIdentityFilter(event.target.value)}><option>전체</option>{audiences.map((item)=><option key={item}>{item}</option>)}<option>미지정</option></select></label><button onClick={()=>{setFromDate("");setToDate("");setIdentityFilter("전체")}}>검색 초기화</button><strong>{filteredUsers.length}명 검색됨</strong></section>
      <div className="admin-grid"><section className="user-panel"><div className="panel-title"><div><p>USER DIRECTORY</p><h2>이용자 목록</h2></div><span>{filteredUsers.length}명</span></div><div className="table-wrap"><table><thead><tr><th>이용자</th><th>신분</th><th>최근 접속</th><th>접근</th><th>영상 열람</th><th>최근 열람 영상</th></tr></thead><tbody>{filteredUsers.length?filteredUsers.map((user)=><tr key={user.email}><td><strong>{user.name||user.email.split("@")[0]}</strong><small>{user.email}</small></td><td><span className={user.identity?"identity-badge":"identity-badge muted"}>{user.identity||"미지정"}</span></td><td>{formatDate(user.lastSeen)}</td><td>{user.accessCount}회</td><td><button className={user.videoViews?"view-count active":"view-count"} disabled={!user.videoViews} onClick={()=>openHistory(user)}>{user.videoViews}건 {user.videoViews>0&&<span>↗</span>}</button></td><td>{user.lastVideo||"—"}</td></tr>):<tr><td colSpan={6} className="empty-cell">검색 조건에 맞는 이용자가 없습니다.</td></tr>}</tbody></table></div></section>
      ...
      <section className="admin-upload-panel"><div className="panel-title"><div><p>NEW CONTENT</p><h2>신규 영상 업로드</h2></div><span>관리자 전용</span></div><div className="admin-upload-form"><label className="wide">영상 파일<input id="admin-video-file" type="file" accept="video/mp4" onChange={(event)=>setFile(event.target.files?.[0]??null)}/><small>{file?`${file.name} · ${(file.size/1024/1024).toFixed(1)}MB`:"MP4 파일을 선택하세요."}</small></label><label>영상 제목<input value={title} onChange={(event)=>setTitle(event.target.value)} placeholder="영상 제목"/></label><label>재생 시간(분)<input type="number" min="1" value={minutes} onChange={(event)=>setMinutes(Math.max(1,Number(event.target.value)))}/></label><label className="wide">영상 소개<textarea value={description} onChange={(event)=>setDescription(event.target.value)} placeholder="이 영상에서 배울 내용을 입력하세요."/></label><label className="wide">태그<input value={tagInput} onChange={(event)=>setTagInput(event.target.value)} placeholder="#신입생 #자료검색 #과제"/><small>#으로 구분하여 여러 태그를 입력할 수 있습니다. 예: #신입생 #자료검색 #과제</small></label><ChoiceGroup title="추천 신분" values={audiences} selected={selectedAudiences} toggle={(value)=>toggle(value,selectedAudiences,setSelectedAudiences)}/><ChoiceGroup title="추천 수준" values={levels} selected={selectedLevels} toggle={(value)=>toggle(value,selectedLevels,setSelectedLevels)}/><ChoiceGroup title="주제" values={topics} selected={selectedTopics} toggle={(value)=>toggle(value,selectedTopics,setSelectedTopics)} wide/><div className="upload-submit wide"><div><div className="progress-track"><i style={{width:`${progress}%`}}/></div><output>{uploadStatus||"신분·수준·주제와 태그를 입력할 수 있습니다."}</output></div><button disabled={uploading} onClick={upload}>{uploading?"업로드 중…":"영상 등록하기"}</button></div></div></section></>}
    </section>
