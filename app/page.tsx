...
export default function Home() {
  ...
  const filtered = useMemo(()=>{
    const query=videoSearch.trim().replace(/^#+/,"").toLocaleLowerCase(lang==="ko"?"ko-KR":"en-US");
    return allVideos.filter((video)=>{
      const matchesTopic=topic==="전체"||(video.topics??[video.topic]).includes(topic);
      if(!matchesTopic||!query)return matchesTopic;
      const titles=[video.title,enVideo[video.title]?.title??""];
      const tags=video.tags??[];
      return [...titles,...tags].some((value)=>value.toLocaleLowerCase(lang==="ko"?"ko-KR":"en-US").includes(query));
    });
  },[allVideos,topic,videoSearch,lang]);
  ...
  const runRecommendation=()=>{ const affiliation=window.localStorage.getItem("hakjeonggwan.affiliation"); void fetch("/api/analytics/profile",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({identity,affiliation})}); setShowResult(true); requestAnimationFrame(()=>document.querySelector("#results")?.scrollIntoView({behavior:"smooth",block:"start"})); };
  ...
  return <main className="dark-site">
    <header className="site-header">
      ...
      <nav aria-label={lang==="ko"?"주요 메뉴":"Main menu"}><a href="#archive">{t.navArchive}</a><a href="#recommend">{t.navRecommend}</a><a href="#guide">{t.navGuide}</a></nav>
...
function VideoCard({video,lang,saved,onSave}:{video:Video;lang:Lang;saved:boolean;onSave:()=>void}){
  const [playing,setPlaying]=useState(false); const t=copy[lang]; const translated=enVideo[video.title]; const title=lang==="ko"?video.title:(translated?.title??video.title); const description=lang==="ko"?video.description:(translated?.description??video.description);
  ...
  return <article className="video-card"><div className={`thumbnail ${video.color} ${playing?"is-playing":""}`}>{playing&&video.mediaKey?<video controls controlsList="nodownload" autoPlay playsInline src={`/media/${encodeURIComponent(video.mediaKey)}`}/>:<><span className="thumbnail-number">{video.mark}</span><button className="play" aria-label={t.playLabel(title)} onClick={playVideo}>▶</button><span className="duration">{video.minutes}:00</span><div className="thumb-lines"><i/><i/><i/></div></>} </div><div className="video-info"><div className="tags"><span>{(video.topics??[video.topic]).map((item)=>lang==="ko"?item:topicNames[item]).join(" · ")}</span><span>{(video.levels??[video.level]).map((item)=>lang==="ko"?item:levelNames[item]).join(" · ")}</span>{video.tags?.length?<span>{video.tags.map((item)=>`#${item}`).join(" ")}</span>:null}</div><h3>{title}</h3><p>{description}</p><div className="card-bottom"><span>{video.audience.slice(0,2).map((item)=>lang==="ko"?item:identityNames[item]).join(" · ")}</span><button className={saved?"save active":"save"} aria-label={saved?t.unsave:t.save} onClick={onSave}>{saved?"♥":"♡"}</button></div></div></article>;
}
