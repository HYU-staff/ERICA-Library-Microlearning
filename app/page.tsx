...
export default function Home() {
  ...
  const filtered = useMemo(()=>{
    const terms=searchQuery.toLocaleLowerCase().split(/\s+/).map((term)=>term.replace(/^#+/,"").trim()).filter(Boolean);
    return allVideos.filter((video)=>{
      const matchesTopic=topic==="전체"||(video.topics??[video.topic]).includes(topic);
      const searchable=[video.title,...(video.tags??[])].join(" ").toLocaleLowerCase();
      return matchesTopic&&terms.every((term)=>searchable.includes(term));
    });
  },[allVideos,topic,searchQuery]);
  ...
  return <main className="dark-site">
    ...
    <section className="archive-section" id="archive"><div className="section-heading archive-heading"><div><p className="kicker">VIDEO ARCHIVE</p><h2>{t.archiveTitle.split("\n").map((line,i)=><span key={line}>{line}{i===0&&<br/>}</span>)}</h2></div><p>{t.archiveDesc.split("\n").map((line,i)=><span key={line}>{line}{i===0&&<br/>}</span>)}</p></div><div className="archive-search"><span aria-hidden="true">⌕</span><input type="search" value={searchQuery} onChange={(event)=>setSearchQuery(event.target.value)} placeholder={lang==="ko"?"영상 제목 또는 #태그 검색":"Search by title or #tag"} aria-label={lang==="ko"?"영상 제목 또는 태그 검색":"Search videos by title or tag"}/>{searchQuery&&<button onClick={()=>setSearchQuery("")} aria-label={lang==="ko"?"검색어 지우기":"Clear search"}>×</button>}</div><div className="topic-tabs" role="tablist" aria-label={lang==="ko"?"영상 주제":"Video topics"}>{topics.map((item)=><button role="tab" aria-selected={topic===item} className={topic===item?"active":""} onClick={()=>setTopic(item)} key={item}>{displayTopic(item)}</button>)}</div>{filtered.length?<div className="video-grid">{filtered.map((video)=><VideoCard key={video.title} video={video} lang={lang} saved={saved.includes(video.title)} onSave={()=>toggleSave(video.title)}/>)}</div>:<div className="archive-empty"><strong>{lang==="ko"?"검색 결과가 없습니다.":"No videos found."}</strong><p>{lang==="ko"?"다른 제목이나 태그로 검색해 보세요.":"Try another title or tag."}</p></div>}</section>
...
function VideoCard({video,lang,saved,onSave}:{video:Video;lang:Lang;saved:boolean;onSave:()=>void}){
  ...
  return <article className="video-card"><div className={`thumbnail ${video.color} ${playing?"is-playing":""}`}>{playing&&video.mediaKey?<video controls controlsList="nodownload" autoPlay playsInline src={`/media/${encodeURIComponent(video.mediaKey)}`}/>:<><span className="thumbnail-number">{video.mark}</span><button className="play" aria-label={t.playLabel(title)} onClick={playVideo}>▶</button><span className="duration">{video.minutes}:00</span><div className="thumb-lines"><i/><i/><i/></div></>} </div><div className="video-info"><div className="tags"><span>{(video.topics??[video.topic]).map((item)=>lang==="ko"?item:topicNames[item]).join(" · ")}</span><span>{(video.levels??[video.level]).map((item)=>lang==="ko"?item:levelNames[item]).join(" · ")}</span></div>{video.tags?.length?<div className="custom-tags">{video.tags.map((tag)=><span key={tag}>#{tag}</span>)}</div>:null}<h3>{title}</h3><p>{description}</p><div className="card-bottom"><span>{video.audience.slice(0,2).map((item)=>lang==="ko"?item:identityNames[item]).join(" · ")}</span><button className={saved?"save active":"save"} aria-label={saved?t.unsave:t.save} onClick={onSave}>{saved?"♥":"♡"}</button></div></div></article>;
}
